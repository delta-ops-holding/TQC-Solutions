...
namespace ApiEmily.Managers
{
    public class TwitchStreamService
    {
        public void Delete(int id)
        {
            ...
            {
                streamerCommand.CommandText = "proc_streamer_delete";
                streamerCommand.CommandType = CommandType.StoredProcedure;
                streamerCommand.Connection = SqlDataAccess.Instance.GetSqlConnection;
            ...
            if (streamerDataReader.HasRows)
            {
                ...
                while (await streamerDataReader.ReadAsync())
                {
                    streamersList.Add(
                        new Streamer()
                        {
                            BaseId = streamerDataReader.GetInt32(0),
                            Name = streamerDataReader.GetString(1),
                            StreamURL = streamerDataReader.GetString(2),
                            StreamerImageURL = streamerDataReader.GetString(3)
                        }
                    );
                }
            }
